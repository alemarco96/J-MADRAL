...
def parse_args() -> argparse.Namespace:
    ...
    args.base_model_name = f"{os.environ['ESCISOTA_DATA']}{os.sep}models{os.sep}bi_encoders{os.sep}init{os.sep}joint-bibert_bert_init"
    # args.base_model_name = f"{os.environ['ESCISOTA_DATA']}{os.sep}models{os.sep}bi_encoders{os.sep}init{os.sep}joint-mtbert_bert_init"
    # args.base_model_name = f"{os.environ['ESCISOTA_DATA']}{os.sep}models{os.sep}bi_encoders{os.sep}init{os.sep}joint-madral_bert_init"

    args.pretrain_alpha = 0.1
    ...
    args.r_corpus_filename = f"{os.environ['ESCISOTA_DATA']}{os.sep}datasets{os.sep}AmazonESCI_Reviews{os.sep}final{os.sep}corpus{os.sep}r{os.sep}joint_r_corpus.jsonl"
    args.s_corpus_filename = f"{os.environ['ESCISOTA_DATA']}{os.sep}datasets{os.sep}AmazonESCI_Reviews{os.sep}final{os.sep}corpus{os.sep}s{os.sep}joint_s_corpus.jsonl"
    args.r_queries_filename = f"{os.environ['ESCISOTA_DATA']}{os.sep}datasets{os.sep}AmazonESCI_Reviews{os.sep}final{os.sep}queries{os.sep}r{os.sep}joint_r_train_queries.jsonl"
    args.s_queries_filename = f"{os.environ['ESCISOTA_DATA']}{os.sep}datasets{os.sep}AmazonESCI_Reviews{os.sep}final{os.sep}queries{os.sep}s{os.sep}joint_s_train_queries.jsonl"

    args.final_model_name = "joint-bibert-tdb_bert_pretrained"
    # args.final_model_name = "joint-mtbert-tdb_bert_pretrained"
    # args.final_model_name = "joint-madral-tdb_bert_pretrained"
...
def main():
    ...
    # ------------------------------------------------------------------------------------------------------------------
    # Read the recommendation corpus data from disk.
    # ------------------------------------------------------------------------------------------------------------------
    r_corpus_data = utils.read_corpus(args.r_corpus_filename, convert_doc_id_to_int=False)
    print("R corpus loaded!", flush=True)
    ...
    # ------------------------------------------------------------------------------------------------------------------
    # Read the recommendation queries data from disk.
    # ------------------------------------------------------------------------------------------------------------------
    r_queries_data = utils.read_corpus(args.r_queries_filename, convert_doc_id_to_int=True)
    print("R queries loaded!", flush=True)
    ...
    train_dataset = modeling.MultiPretrainDataset(
        corpora_data=[r_corpus_data, s_corpus_data, r_queries_data, s_queries_data],
        corpora_samples_per_epoch=[len(r_corpus_data), len(s_corpus_data) // args.num_train_epochs,
                                   len(r_queries_data), len(s_queries_data)]
    )

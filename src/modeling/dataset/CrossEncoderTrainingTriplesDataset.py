...
class CrossEncoderTrainingTriplesDataset(datasets.Dataset, transformers.TrainerCallback):
    def __init__(self,
                 corpus_data: dict[int | str, dict[str, str | list[int]]],
                 triples_data: dict[int | str, dict[str, str | list[int] | dict[int | str, int]]],
                 num_retain_hardnegatives: int = 10,
        ...
        self.train_data = {k1: {
            "e_id": sorted(v1["positives_id"].items(), key=operator.itemgetter(1), reverse=False),
            "s_id": sorted([(k2, v2) for k2, v2 in v1["negatives_id"].items() if k2 in v1["s_documents"]],
                           key=operator.itemgetter(1), reverse=False),
            "c_id": sorted([(k2, v2) for k2, v2 in v1["negatives_id"].items() if k2 in v1["c_documents"]],
                           key=operator.itemgetter(1), reverse=False),
            "i_id": sorted([(k2, v2) for k2, v2 in v1["negatives_id"].items() if k2 in v1["i_documents"]],
                           key=operator.itemgetter(1), reverse=False),
            "hn_id": sorted(v1["hardnegatives_id"].items(), key=operator.itemgetter(1),
                            reverse=False)[:num_retain_hardnegatives]
        } for k1, v1 in triples_data.items()}
        self.train_data = {k: v for k, v in self.train_data.items() if len(v["e_id"]) >= 1 and
        ...
        self.num_soft_positives = num_soft_positives
    ...
    def __getitem__(self, idx):
        ...
        for i1 in idx:
            ...
            poss_id = [(k, self.exact_label) for k, _ in query_triple["e_id"]]
            if len(poss_id) > 0:
                poss_id = random.sample(poss_id, min(len(poss_id), self.num_positives))
            else:
                poss_id = []
            num_soft = self.num_soft_positives + (self.num_positives - len(poss_id))

            # Select the soft positive document(s).
            soft_id = [(k, self.substitute_label) for k, _ in query_triple["s_id"]] + \
                      [(k, self.complementary_label) for k, _ in query_triple["c_id"]] + \
                      [(k, self.irrelevant_label) for k, _ in query_triple["i_id"]]
            if len(soft_id) > 0:
                soft_id = random.sample(soft_id, min(len(soft_id), num_soft))
            else:
                soft_id = []
            num_hard = self.num_hard_negatives + (self.num_soft_positives - len(soft_id)) + \
                       (self.num_positives - len(poss_id))

            # Select the hard negative document(s).
            hard_id = [(k, self.hardnegative_label) for k, _ in query_triple["hn_id"]]
            if len(hard_id) > 0:
            ...
            num_rand = self.num_random_negatives + (self.num_hard_negatives - len(hard_id)) + \
                       (self.num_soft_positives - len(soft_id)) + (self.num_positives - len(poss_id))

            # Sample the random negative document(s).
            rand_id = random.sample(self.corpus_keys, num_rand)
            rand_id = [(k, self.random_label) for k in rand_id]
            del query_triple, num_soft, num_hard, num_rand

            # Aggregate all documents id and their associated relevance label.
            train_data = poss_id + soft_id + hard_id + rand_id
            random.shuffle(train_data)
            del poss_id, soft_id, hard_id, rand_id

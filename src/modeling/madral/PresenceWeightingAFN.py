...
class PresenceWeightingAFN(AFNModule):
    def __init__(self, embedding_size: int, num_aspects: int):
        super(PresenceWeightingAFN, self).__init__(embedding_size, num_aspects)
        ...
        # self.afn_aspect_gamma = torch.nn.Parameter(torch.full([self.num_aspects], 1.0 / self.num_aspects))
    ...
    def forward(self, aspects_embedding: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        presence_logits = torch.stack([self.afn_presence_linear[i](aspects_embedding[:, i, :]).squeeze(1)

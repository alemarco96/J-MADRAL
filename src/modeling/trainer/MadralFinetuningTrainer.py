...
class MadralFinetuningTrainer(transformers.Trainer):
    def __init__(self, *args, finetune_alpha: float, auxiliary_alpha: float, logging_filename: str, **kwargs):
        super(MadralFinetuningTrainer, self).__init__(*args, **kwargs)
        self.finetune_alpha = finetune_alpha
        self.auxiliary_alpha = auxiliary_alpha
    ...
    def compute_loss(
        ...
        # Scale the CE loss according to the task.
        if self.auxiliary_alpha != 0.0:
            task_alpha = 1.0 if inputs["task"][0] in {"r", "s"} else self.auxiliary_alpha
            ce_loss *= task_alpha
            del task_alpha
